...
public class AbstractSummarizationSelectorHits {
  ...
  private DirectedSparseGraph<Node, String> g = new DirectedSparseGraph<>();
  ...
  protected List<Node> getHighRankNdes() {
    ...
    double total_weight =
        reversedNodeList.parallelStream()
            .filter(node -> node.getLevel() == 1)
            .map(node -> node.getAuthorityWeight())
  ...
  /**
   * Get all the nodes from the graph and create a list after sorting them based on Authority Weight
   * in descending order
   *
  ...
  protected ArrayList<Node> sortAndReverse(DirectedSparseGraph<Node, String> g) {
    ...
    Collections.sort(
        orderedList,
            Comparator.comparingDouble(Node::getAuthorityWeight));
